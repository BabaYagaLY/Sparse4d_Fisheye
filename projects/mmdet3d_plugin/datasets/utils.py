...
def draw_lidar_bbox3d_on_fisheye_img(
    bboxes3d, raw_img, camera_intrinsic, camera_extrinsic, camera_distortion,
    color=(0, 255, 0), thickness=2, n_pts_per_edge=30,
):
    """Project 3D bbox to fisheye image with dense edge sampling for correct curves.

    Unlike pinhole cameras where 3D straight lines remain straight in 2D,
    fisheye equidistant projection curves 3D lines. This function densely
    samples each 3D box edge, projects every sample point, and draws a
    polyline to approximate the true fisheye curve.

    Args:
        bboxes3d: LiDARInstance3DBoxes in lidar coordinate system
                  (nuScenes format: [x, y, z, w, l, h, yaw, ...]).
        raw_img: numpy array of the fisheye image (H, W, 3).
        camera_intrinsic: (3, 3) K matrix.
        camera_extrinsic: (4, 4) lidar-to-camera homogeneous transform.
        camera_distortion: (4,) [k1, k2, k3, k4].
        color: color or list of colors.
        thickness: line thickness.
        n_pts_per_edge: number of sample points per 3D edge.
    """
    img = raw_img.copy()
    h, w = img.shape[:2]
    ...
    R_l2c = camera_extrinsic[:3, :3].astype(np.float64)
    t_l2c = camera_extrinsic[:3, 3].astype(np.float64)

    fx = float(camera_intrinsic[0, 0])
    fy = float(camera_intrinsic[1, 1])
    cx = float(camera_intrinsic[0, 2])
    cy = float(camera_intrinsic[1, 2])
    k = np.array(camera_distortion, dtype=np.float64)
    ...
    for bi in range(num_bbox):
        box_corners_ego = corners_3d[bi]
        c = color[bi] if not isinstance(color[0], (int, np.integer)) else color

        for start, end in line_indices:
            p0 = box_corners_ego[start]
            p1 = box_corners_ego[end]
            ...
            # Fisheye equidistant projection (same formula as visualize_dataset.py)
            x, y_c, z = samples_cam[:, 0], samples_cam[:, 1], samples_cam[:, 2]
            r = np.sqrt(x * x + y_c * y_c) + 1e-12
            ...
            pts = np.stack([u[in_frame], v[in_frame]], axis=-1).astype(np.int32)
            cv2.polylines(img, [pts.reshape((-1, 1, 2))], False, c,
