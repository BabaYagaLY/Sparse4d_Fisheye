...
from .utils import (
    ...
    draw_lidar_bbox3d_on_fisheye_img,
...
class NuScenes3DDetTrackDataset(Dataset):
    ...
    def load_annotations(self, ann_file):
        ...
        print(self.metadata)
        # Load fisheye calibration if available (for camera_distortion injection)
        self._calib_D = None
        self._calib_camera_order = None
        calib_npz = os.path.join(os.path.dirname(ann_file), "fisheye_calib_4cam.npz")
        if os.path.exists(calib_npz):
            calib = np.load(calib_npz, allow_pickle=True)
            self._calib_D = calib["D"]                 # (4, 4) [k1,k2,k3,k4]
            self._calib_K = calib["K"]                 # (4, 3, 3)
            self._calib_camera_order = calib["camera_order"]  # e.g. VCAMERA_FISHEYE_*
            print(f"Loaded fisheye calibration: D shape={self._calib_D.shape}, "
                  f"cameras={self._calib_camera_order}")
        return data_infos
    ...
    def get_data_info(self, index):
        ...
        if self.modality["use_camera"]:
            ...
            for cam_idx, (cam_type, cam_info) in enumerate(info["cams"].items()):
                ...
                # Inject fisheye distortion coefficients from calibration
                if self._calib_D is not None and cam_idx < len(self._calib_D):
                    cam_distortion.append(self._calib_D[cam_idx].copy())
                else:
                    cam_distortion.append(np.zeros(4, dtype=np.float32))
    ...
    def get_ann_info(self, index):
        ...
        if "gt_visibility" in info:
            anns_results["gt_visibility"] = info["gt_visibility"][mask]
    ...
    def _compute_ap(recall, precision):
        """Compute VOC AP from precision-recall curve."""
    ...
    def _evaluate_custom(
        ...
        """
        Custom evaluation for fisheye dataset without NuScenes metadata.
        Computes per-class AP using center-distance matching.
        """
        ...
        # Gather predictions and GT per class
        per_class_pred = {c: [] for c in range(num_classes)}
        per_class_gt = {c: 0 for c in range(num_classes)}

        for sample_id, result in enumerate(results):
            if "img_bbox" in result:
                result = result["img_bbox"]
            pred_boxes = result["boxes_3d"].numpy()
            ...
            num_pts = info.get("num_lidar_pts", np.ones(len(gt_boxes)))
            ...
            for c in range(num_classes):
                ...
                per_class_gt[c] += len(cls_gt_boxes)
                ...
                for box, score in zip(cls_pred_boxes, cls_pred_scores):
                    # Find nearest GT of the same class
                    if len(cls_gt_boxes) > 0:
                        dists = np.linalg.norm(
                            cls_gt_boxes[:, :3] - box[None, :3], axis=1
                        )
                        matched = dists.min() < distance_threshold
                        gt_idx = int(dists.argmin())
                    else:
                        matched = False
                        gt_idx = -1
                    per_class_pred[c].append({
                        "score": float(score),
                        "matched": matched,
                        "gt_idx": gt_idx,
                        "sample_id": sample_id,
        ...
        for c, name in enumerate(class_names):
            preds = per_class_pred[c]
            num_gt = per_class_gt[c]
            if num_gt == 0:
                detail[f"{metric_prefix}/{name}_AP"] = 0.0
                continue

            preds = sorted(preds, key=lambda x: x["score"], reverse=True)
            tp = np.zeros(len(preds))
            fp = np.zeros(len(preds))
            matched_gt = {}  # (sample_id, gt_idx) -> bool

            for i, pred in enumerate(preds):
                if not pred["matched"]:
                    fp[i] = 1
                    continue
                key = (pred["sample_id"], pred["gt_idx"])
            ...
            recalls_curve = tp_cumsum / num_gt
            precisions = tp_cumsum / (tp_cumsum + fp_cumsum + 1e-12)

            ap = self._compute_ap(recalls_curve, precisions)
            aps.append(ap)
            recalls.append(recalls_curve[-1] if len(recalls_curve) > 0 else 0.0)

            detail[f"{metric_prefix}/{name}_AP"] = round(ap, 4)
            detail[f"{metric_prefix}/{name}_ATE"] = 0.0
            detail[f"{metric_prefix}/{name}_ASE"] = 0.0
            detail[f"{metric_prefix}/{name}_AOE"] = 0.0
            detail[f"{metric_prefix}/{name}_AVE"] = 0.0
            detail[f"{metric_prefix}/{name}_AAE"] = 0.0

        mAP = np.mean(aps) if len(aps) > 0 else 0.0
        mean_recall = np.mean(recalls) if len(recalls) > 0 else 0.0
        # NDS-like score: weighted combination
        nds = 0.5 * mAP + 0.5 * mean_recall
        detail[f"{metric_prefix}/mAP"] = round(mAP, 4)
        detail[f"{metric_prefix}/NDS"] = round(nds, 4)
        detail[f"{metric_prefix}/mATE"] = 0.0
        detail[f"{metric_prefix}/mASE"] = 0.0
        detail[f"{metric_prefix}/mAOE"] = 0.0
        detail[f"{metric_prefix}/mAVE"] = 0.0
        detail[f"{metric_prefix}/mAAE"] = 0.0

        print(f"Custom evaluation: mAP={mAP:.4f}, NDS={nds:.4f}")
        return detail
    ...
    def evaluate(
        ...
        # For custom fisheye datasets without NuScenes metadata, use custom eval.
        if self.version.startswith("fisheye"):
            results_dict = self._evaluate_custom(
                results, result_name=result_names[0]
            )
            if show or out_dir:
    ...
    def show(self, results, save_dir=None, show=False, pipeline=None):
        ...
        for i, result in enumerate(results):
            ...
            # Detect fisheye mode from dataset calibration
            use_fisheye = self._calib_D is not None
            if use_fisheye:
                # Build per-camera fisheye params for this frame
                ...
                for cam_idx, (cam_type, cam_info) in enumerate(info["cams"].items()):
                    ...
                    ext[:3, :3] = R_l2c
                    ext[:3, 3] = t_l2c
                    camera_extrinsics.append(ext)
                    if cam_idx < len(self._calib_D):
                        camera_distortions.append(self._calib_D[cam_idx].copy())
                    else:
                        camera_distortions.append(np.zeros(4, dtype=np.float64))

            for j, img_origin in enumerate(raw_imgs):
                img = img_origin.copy()
                if len(pred_bboxes_3d) != 0:
                    if use_fisheye:
                        img = draw_lidar_bbox3d_on_fisheye_img(
                            pred_bboxes_3d, img,
                            camera_intrinsics[j],
                            camera_extrinsics[j],
                            camera_distortions[j],
            ...
            # Dynamic layout: 2 columns for any number of cameras
            num_imgs = len(imgs)
            cols = 2
            rows = (num_imgs + cols - 1) // cols
            row_imgs = []
            for r in range(rows):
                row_cams = imgs[r*cols:(r+1)*cols]
                # Pad with black if needed
                while len(row_cams) < cols:
                    row_cams.append(np.zeros_like(imgs[0]))
                row_imgs.append(np.concatenate(row_cams, axis=1))
            image = np.concatenate(row_imgs, axis=0)
