    ...
    def __call__(self, results):
        ...
        for i in range(N):
            ...
            if "cam_intrinsic" in results:
                # Apply the full 2D affine transform (resize + crop + flip + rotate)
                # to the intrinsic matrix so that K[2, 2] stays 1 and the
                # principal point is correctly shifted. The old code only scaled
                # by resize, which corrupted K[2, 2] and produced wrong
                # lidar2cam extrinsics for fisheye projection.
