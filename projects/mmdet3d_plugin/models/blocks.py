...
class DeformableFeatureAggregation(BaseModule):
    def __init__(
        ...
        if use_camera_embed:
            if projection_mode == "fisheye":
                from .fisheye_projection import build_fisheye_camera_encoder
                self.camera_encoder = build_fisheye_camera_encoder(embed_dims, in_channels=8)
            else:
    ...
    def forward(
        ...
        **kwargs: dict,
        ...
        # For fisheye, also compute per-camera "in front" mask so that points
        # behind a camera do not contribute to that camera's deformable attention.
        if self.projection_mode == "fisheye":
            pts_cam = _transform_lidar_to_camera(
                key_points.float(),
                metas["camera_extrinsic"].float(),
            )
            in_front = pts_cam[..., 2] > 0  # (bs, num_cams, num_anchor, num_pts)
        else:
            in_front = None
        ...
        if self.use_deformable_func:
            points_2d = (
                self.project_points(
                    ...
                    metas.get("image_wh"),
                    camera_intrinsic=metas.get("camera_intrinsic"),
                    camera_distortion=metas.get("camera_distortion"),
                    camera_extrinsic=metas.get("camera_extrinsic"),
                )
                .permute(0, 2, 3, 1, 4)
            ...
            # Only cameras where the keypoint projects inside the image
            # and is in front of the camera should contribute.
            in_image = (
                (points_2d >= 0.0) & (points_2d <= 1.0)
            ).all(-1)
            if in_front is not None:
                in_image = in_image & in_front.permute(0, 2, 3, 1)
            in_image = in_image[..., None, None]
            weights = weights * in_image
            weights_sum = weights.sum(dim=(2, 3, 4), keepdim=True).clamp(min=1e-12)
            ...
            in_image = (
                (points_2d >= 0.0) & (points_2d <= 1.0)
            ).all(-1).permute(0, 2, 1, 3)
            if in_front is not None:
                in_image = in_image & in_front.permute(0, 2, 1, 3)
            in_image = in_image[:, :, :, None, :, None]
            weights = weights * in_image
            weights_sum = weights.sum(dim=(2, 3, 4), keepdim=True).clamp(min=1e-12)
            weights = weights / weights_sum
    ...
    def _get_weights(self, instance_feature, anchor_embed, metas=None):
        ...
        if self.camera_encoder is not None:
            if self.projection_mode == "fisheye":
                K = metas["camera_intrinsic"][:, :, :3]
                D = metas.get("camera_distortion",
                    torch.zeros(bs, self.num_cams, 4, device=instance_feature.device))
                camera_params = torch.cat([
                    K[:, :, 0, 0:1], K[:, :, 1, 1:2],
                    K[:, :, 0, 2:3], K[:, :, 1, 2:3],
                    D[:, :, :4],
                ], dim=-1)
                camera_embed = self.camera_encoder(camera_params)
            else:
    ...
    def project_points(self, key_points, projection_mat, image_wh=None,
        ...
        if self.projection_mode == "fisheye":
            ...
            return fisheye_project(
                key_points, camera_extrinsic, camera_intrinsic,
                camera_distortion, image_wh,
            )
    ...
    def feature_sampling(
        ...
        camera_distortion: Optional[torch.Tensor] = None,
        ...
        points_2d = self.project_points(
            ...
            camera_intrinsic, camera_distortion, camera_extrinsic,
        )
        points_2d = points_2d * 2 - 1

        # === Safety: mask NaN/Inf coordinates to off-grid (-10) before grid_sample ===
        invalid_mask = torch.isnan(points_2d) | torch.isinf(points_2d)
        points_2d = torch.where(
            invalid_mask,
            points_2d.new_tensor(-10.0),
            points_2d
        )
        # =============================================================================
