"""
Run inference on selected frames using a checkpoint and visualize
predictions on fisheye images with per-view visibility filtering.

Supports train/val/test splits via --split.
...
Usage:
    ...
    python tools/visualize_predictions.py \
        ...
        --split val \
        --out-dir   work_dirs/fisheye_sparse4d_unfreeze_v2_120k_ffn_refine/visual_val_pred\
        --num-frames 20 \
        --score-thr 0.05
"""
import argparse
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Visualize model predictions on fisheye images."
    )
    parser.add_argument("config", help="test config file path")
    parser.add_argument("checkpoint", help="checkpoint file")
    parser.add_argument("--split", type=str, default="val", choices=["train", "val", "test"],
                        help="dataset split to visualize")
    parser.add_argument("--out-dir", default="work_dirs/fisheye_sparse4d/visual_pred",
...
def main():
    ...
    # Build dataset from the requested split
    ds_cfg = cfg.data.test.copy()
    if args.split == "train":
        ds_cfg["ann_file"] = cfg.data.train["ann_file"]
    elif args.split == "val":
        ds_cfg["ann_file"] = cfg.data.val["ann_file"]
    elif args.split == "test":
        ds_cfg["ann_file"] = cfg.data.test["ann_file"]
